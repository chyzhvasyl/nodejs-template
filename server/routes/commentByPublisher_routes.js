...
// *** api routes *** //
router.get('/commentsByPublisher/:flag', findAllCommentsByPublisher);
router.get('/commentByPublisher/:id', findCommentByPublisherById);
router.get('/commentsByPublisher/:confirmation/:flag', findCommentsByPublisherByConfirmation);
...
function findAllCommentsByPublisher(req, res, next) {
	passport.authenticate('local', function(err, user) {
		...
		if (user && user.roles && user.roles.includes('CN=NEWS_publisher')) { 
			CommentByPublisher.find().sort({ time : -1 }).skip(req.params.flag * dataChunk).limit(dataChunk)
				.populate('article')
...
function findCommentsByPublisherByConfirmation(req, res, next) {
	passport.authenticate('local', function(err, user) {
		if (err) { return next(err); }
		if (user && user.roles && user.roles.includes('CN=NEWS_publisher')) { 
			CommentByPublisher.find({'confirmation':req.params.confirmation}).sort({ time : -1 }).skip(req.params.flag * dataChunk).limit(dataChunk)
				.populate('article')
				.exec(function(err, commentsByPublisher) {
					if(err) {
						res.status(400);
